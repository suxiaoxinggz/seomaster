...
async function dfsRequest(endpoint: string, payload: any[], config: SeoConfig) {
    if (!config.apiLogin || !config.apiPassword) {
        throw new Error("Missing API Credentials. Please configure them in Settings.");
    }
    const auth = btoa(`${config.apiLogin}:${config.apiPassword}`);
    
    // Use the custom Base URL if provided in config, else default
    ...
    try {
        ...
        if (!response.ok) {
            ...
            // Extract meaningful message from DFS error structure
        ...
        // DataForSEO 200 OK doesn't mean success. Check internal status_code.
        // status_code 20000 = OK.
        if (data.status_code !== 20000) {
             const taskError = data.tasks?.[0]?.status_message || data.status_message;
             throw new Error(`DataForSEO Task Failed: ${taskError}`);
        }
        
        return data.tasks;

    } catch (error) {
        console.error(`DFS Request Failed [${endpoint}]:`, error);
        throw error;
    }
}
